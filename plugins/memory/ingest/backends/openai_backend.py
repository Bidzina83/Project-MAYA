...
class OpenAIBackend:
    ...
    def __init__(self, model: Optional[str] = None, max_retries: int = 5, backoff_factor: float = 0.5):
        ...
        if self.api_key:
            ...
            # Prefer legacy module binding if a legacy-compatible 'openai' is present.
            if openai is not None and hasattr(openai, 'Embedding'):
                ...
                # Fall back to new-style client if available
                if OpenAIClient is not None:
                    try:
                        self._client = OpenAIClient(api_key=self.api_key)
                    except Exception:
                        self._client = None
                # If new client not used, try legacy module again
